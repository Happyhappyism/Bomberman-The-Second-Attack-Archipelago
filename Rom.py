...
def write_tokens(world:World, patch:BombTSAProcedurePatch, startbomb):
    ...
    # Skip tutorial to avoid crash
    #patch.write_token(APTokenTypes.WRITE, 0x386BC, bytes([0x10,0x00,0x00,0x13]))
    patch.write_token(APTokenTypes.WRITE, 0x38708, bytes([0xAE,0x60,0x19,0xF8])) # SW R0, 0x19F8 (S3) [80037B08]
    # Setup GP
    ...
    # ROM Adjustment Data
    adjust_option_set = [
        world.options.chara_model.value,
        world.options.guardian_helmet.value,
        world.options.guardian_vest.value,
        world.options.guardian_arms.value,
        world.options.guardian_legs.value,
        ]
    adj_offset = ADJUST_OFFSET + 0x10
    for file_name in adjust_option_set:
        patch.write_token(APTokenTypes.WRITE, adj_offset, file_name.encode("ascii"))
        adj_offset += 0x8
