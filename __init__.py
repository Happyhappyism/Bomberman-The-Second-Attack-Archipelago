...
from .tracker import populate_location_json
import json
...
class BombTSAWorld(World):
    ...
    def set_rules(self) -> None:
        ...
        for location in self.multiworld.get_locations(player):
            ...
            if name in location_rules:
                ...
                if name in dark_bomb_exc_list:
                    add_rule(location, lambda state: can_destroy_with_immunity(state, player, ["Dark Stone"]))
    ...
    def fill_slot_data(self) -> Dict[str, Any]:
        return {
            ...
            #"powersanity": self.options.chara_model.value,
            "chara_model": self.options.chara_model.value,
            "guardian_helmet": self.options.guardian_helmet.value,
            "guardian_vest": self.options.guardian_vest.value,
            "guardian_arms": self.options.guardian_arms.value,
            "guardian_legs": self.options.guardian_legs.value,
            "random_efx": self.options.random_efx.value,
        }
    ...
    def generate_output(self, output_directory: str):
        ...
        #with open(os.path.join(output_directory, f"{out_file_name}_tracker.json"), "w") as f:
        #    json.dump(populate_location_json(), f) # No longer needed, dumps tracker information