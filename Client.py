...
class BombTSAClient(BizHawkClient):
    ...
    async def validate_rom(self, ctx: "BizHawkClientContext") -> bool:
        ...
        rom_data = await bizhawk.read(
            ...
            (ADJUST_OFFSET, 0x40, "ROM"), # 6 Adjust Data
            ]
        )
        ...
        #self.adjusted = game_options[0xA]
        ...
        ctx.adjusted = rom_data[1][0xA]
        adjust_data = rom_data[6]
        if ctx.adjusted == 0 and adjust_data[0] == 0:
            #logger.warning(f"{ctx.slot_data}")
            adjust_rom(adjust_data)
            ctx.adjusted = 1
    ...
    async def game_watcher(self, ctx: "BizHawkClientContext") -> None:
        ...
        try:
            ram_data = await bizhawk.read(
                ...
                (0xB569F,0x12, "RDRAM"), # C menu state?
            ...
            # if not self.adjusted and ctx.slot_data is not None:
            #     logger.warning(f"{ctx.slot_data}")
            #     adjust_rom(ctx.slot_data)
            #     self.adjusted = True
            ...
            if self.pending_death_link:
                ...
                self.sending_death_link = True # Huh?
                ...
                try: 
                    if len(ary) >= 3:
                        ...
                        if (loc_id in ctx.server_locations) and flag:
                            #logger.warning(f"flag location - offset: {hex(offset)}, address: {hex(0xAC2C0 + offset)}, mask: {hex(ary[1])}, loc_id: {hex(loc_id)}, flag:{hex(flag)}")
                        ...
                        if mapid == mapexcept:
                            ...
                            if (loc_id in ctx.server_locations) and flag:
                                #logger.warning(f"flag location map except - offset: {hex(offset)}, address: {hex(0xAC2C0 + offset)} mask: {hex(ary[1])}, loc_id: {hex(loc_id)}, flag:{hex(flag)}")
                                locs_to_send.add(loc_id)
                except IndexError as e:
                    logger.warning(f"Error checking flag {e}")
            ...
            if inshop == 0x800B57C0:
                for loc_id, ary in SHOP_MAP.items():
                    ...
                    if flag and (loc_id not in ctx.checked_locations) and (loc_id in ctx.server_locations):
                        #logger.warning(f"shop location - offset: {hex(offset)}, address: {hex(0xAC2C0 + offset)} mask: {hex(mask)}, loc_id: {hex(loc_id)}, flag:{hex(flag)}")
                        locs_to_send.add(loc_id)
            ...
            if full_power & 0x40:
                ...
                outbound_writes.append((0xAC2E9, bytearray([clear_fp]), "RDRAM"))

            # Stagger item writes
            match self.timer_tick:
                case 1:
